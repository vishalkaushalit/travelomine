...
class BookingExportController extends Controller
{
    ...
    public function exportMultipleSelected(Request $request)
{
    $request->validate([
    ...
    // ✅ Load all bookings with eager loading
    ...
    // ✅ Create CSV file
    $filename = 'mis-bookings-export-' . date('Y-m-d_H-i-s') . '.csv';
    ...
    return response()->stream(function () use ($bookings) {
        ...
        // ✅ Write header row
        fputcsv($handle, [
            'Timestamp', 'Date', 'Booking Reference', 'Agent Name', 'Call Type',
            'Manager', 'Airline', 'Sector', 'GK PNR', 'Airline PNR',
            'Travel Date', 'Verticals', 'Service Provided', 'Booking Portal', 'Card Holder Name',
            'Any Passenger Name', 'Card Last 4 digit', 'Calling Number', 'Billing Phone Number',
            'Email Address', 'Booking Status', 'Email - Auth Taken', 'Merchant', 'Currency',
            'Total Quoted', 'Amount Charged', 'Amount paid to airline', 'Total MCO', 'Language',
            'Company card/VAN (if used with amount)', 'Agent remarks if any', 'Cabin',
            'Email ID Used for Airline Conf (If any)', 'Return Date', 'Trip Details',
            'Campaign', 'Publisher', 'Target', 'Remarks By MIS', 'Merchant Remarks by MIS',
            'Transaction Status', 'Ticket Status', 'Merchant Match', 'Fare Type (NFXR / FXR)',
            'Amadeus Pseudo', 'Issuance Fee ($10/PP)', 'Refund // Void Amount', 'MCO Match',
            'Charging E-Ticket', 'Updated in Company Kitty', 'Amount Charged in CAD//AUD'
        ]);
