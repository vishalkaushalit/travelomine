...
@section('content')
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <div class="mb-3">
                        <label for="lookup_pnr" class="form-label">Enter GK PNR / Airline PNR <span
                                class="text-danger">*</span></label>
            ...
            <div class="modal-content">
                <div class="modal-header bg-primary text-white">
                    <h5 class="modal-title">
                        <i class="bi bi-plus-circle"></i> Add Booking Remark
                    </h5>
                ...
                <div class="modal-body">
                    <form id="addRemarkForm">
                        @csrf
                        <input type="hidden" id="remark_booking_id" value="">

                        <div class="mb-3">
                            <label class="form-label">Remark Type</label>
                            <select id="remark_type" class="form-control" required>
                        ...
                        <div class="mb-3">
                            ...
                            <textarea id="remark_text" rows="4" class="form-control" placeholder="Enter your remark here..." required></textarea>
                        </div>

                        <div class="mb-3">
                            <label class="form-label">Amount Changed (if applicable)</label>
                            <input type="number" id="amount_changed" step="0.01" class="form-control"
                                placeholder="0.00">
                        </div>
                    </form>
                </div>
                <div class="modal-footer">
                    ...
                    <button type="button" class="btn btn-primary" id="saveRemarkBtn">
                        <i class="bi bi-save"></i> Add Remark
                    </button>
                </div>
            </div>
        </div>
    ...
    {{-- Include Remarks Timeline on Booking Show Page --}}
    <div class="card mt-4" id="remarksSection">
        <div class="card-body">
            <div id="remarksTimeline">
            ...
            async function searchBookingByPnr() {
                ...
                try {
                    ...
                    if (serviceProvidedSelect) {
                        serviceProvidedSelect.dispatchEvent(new Event('change', {
                            bubbles: true
                        }));
                    }

                    showMessage('success',
                        '✓ Booking found! Form has been prefilled. Please review and update payment details.'
                    );
                    lookupLocked = true;
                    ...
                    lookupLocked = false;
                    showMessage('warning', 'No existing booking found for PNR: ' + pnr +
                        '. You can manually enter all details for this new booking.');
...
@push('scripts')
    <script>
        ...
        function loadRemarks(bookingId) {
            ...
            fetch(`/agent/bookings/${bookingId}/remarks`, {
                ...
                .catch(error => {
                    ...
                    document.getElementById('remarksTimeline').innerHTML =
                        '<div class="alert alert-danger">Failed to load remarks</div>';
                });
        }
        ...
        function renderRemarks(remarks) {
            if (!remarks || remarks.length === 0) {
                document.getElementById('remarksTimeline').innerHTML =
                    '<div class="text-center py-4"><i class="bi bi-chat-square-text display-4 text-muted"></i><p class="text-muted mt-2">No remarks added yet.</p></div>';
                return;
            }

            let html = `
        <div class="d-flex justify-content-between align-items-center mb-3">
            <h5 class="mb-0"><i class="bi bi-chat-dots"></i> Booking Timeline & Remarks (${remarks.length})</h5>
            <button type="button" class="btn btn-sm btn-primary" data-bs-toggle="modal" data-bs-target="#addRemarkModal">
    ...
    `;
            ...
            remarks.forEach((remark, index) => {
                const isLegacy = remark.is_legacy || false;
                const badgeColor = remark.remark_type === 'payment' ? 'success' :
                    (remark.remark_type === 'modification' ? 'warning' : 'secondary');

                html += `
            <div class="timeline-item ${index === 0 ? 'active' : ''}">
                <div class="timeline-badge">
                    <i class="bi ${getRemarkIcon(remark.remark_type)}"></i>
                </div>
                <div class="timeline-panel">
                    <div class="timeline-heading">
                        <div class="d-flex justify-content-between">
                            <h6 class="timeline-title">
                                <span class="badge bg-${badgeColor}">${remark.remark_type.toUpperCase()}</span>
                                ${isLegacy ? '<span class="badge bg-info ms-1">Original</span>' : ''}
                            </h6>
                            <small class="text-muted">
                                <i class="bi bi-clock"></i> ${new Date(remark.created_at).toLocaleString()}
                            </small>
                        </div>
                        <small><i class="bi bi-person"></i> Added by: ${remark.agent?.name || 'System'}</small>
                    </div>
                    <div class="timeline-body">
                        <p class="mb-2">${remark.remark_text.replace(/\n/g, '<br>')}</p>
                        ${remark.amount_changed ? `<div class="alert alert-warning py-1 px-2 mb-0 d-inline-block"><i class="bi bi-cash"></i> Amount: $${parseFloat(remark.amount_changed).toFixed(2)}</div>` : ''}
                        ${!isLegacy && remark.ip_address ? `<small class="text-muted d-block mt-1"><i class="bi bi-hdd-network"></i> IP: ${remark.ip_address}</small>` : ''}
                    </div>
                </div>
            </div>
        `;
            ...
            html += `</div>`;
            document.getElementById('remarksTimeline').innerHTML = html;
        }
        ...
        document.getElementById('saveRemarkBtn')?.addEventListener('click', function() {
            const bookingId = document.getElementById('remark_booking_id').value;
            const remarkText = document.getElementById('remark_text').value;
            const remarkType = document.getElementById('remark_type').value;
            const amountChanged = document.getElementById('amount_changed').value;
            ...
            fetch(`/agent/bookings/${bookingId}/add-remark`, {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                ...
                .then(data => {
                    if (data.success) {
                        ...
                        document.getElementById('amount_changed').value = '';
                        // Reload remarks
        ...
        // When showing remark modal, set booking ID
        document.getElementById('addRemarkModal')?.addEventListener('show.bs.modal', function(event) {
            ...
            if (bookingId) {
                document.getElementById('remark_booking_id').value = bookingId;
            }
        });
    </script>
@endpush
