                    ...
                    <div class="card-body">
                        <form action="{{ route('admin.settings.store') }}" method="POST" class="mb-3">
                            ...
                            <div class="input-group">
                                <input type="text" name="value" class="form-control" placeholder="Add new option"
                                    required>
                        ...
                        <ul class="list-group">
                            @forelse($serviceProvided as $id => $value)
                                <li class="list-group-item d-flex justify-content-between align-items-center">
                                    {{ $value }}
                                    <form action="{{ route('admin.settings.destroy', ['key' => 'service_provided', 'id' => $id]) }}" method="POST"
                    ...
                    <div class="card-body">
                        ...
                        <ul class="list-group">
                            @forelse($serviceType as $id => $value)
                                <li class="list-group-item d-flex justify-content-between align-items-center">
                                    {{ $value }}
                                    <form action="{{ route('admin.settings.destroy', ['key' => 'service_type', 'id' => $id]) }}" method="POST"
                                        style="display:inline;">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="btn btn-sm btn-danger"
