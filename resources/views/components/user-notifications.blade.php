    ...
    if ($user) {
        ...
        $notifications = AdminNotification::where('is_active', true)
            ->where(function ($query) use ($user) {
                $query->where('target_type', 'all')->orWhereJsonContains('target_roles', $user->role);
            })
        ...
        // Show only the latest notification on dashboard
        $notifications = $notifications->take(1);
    } catch (\Exception $e) {
        ...
        $(document).ready(function() {
            ...
            $('.alert-dismissible').on('closed.bs.alert', function() {
                ...
                if (canDismiss) {
                    ...
                    let url = '{{ route('notifications.read', ['id' => ':id']) }}'.replace(':id',
                        notificationId);
