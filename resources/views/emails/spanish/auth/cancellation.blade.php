    <h3>Confirmación de autorización de cancelación de {{ $booking->segments->first()?->airline_name ?? 'la aerolínea' }}.</h3>
    <p>Estimado/a {{ $booking->customer_name ?? 'Pasajero' }},</p>
    <p>¡Reciba un cordial saludo!</p>
    <p>Según nuestra conversación y lo acordado, hemos procesado la cancelación con
        {{ $booking->segments->first()?->airline_name ?? 'la aerolínea' }} bajo la
        confirmación {{ $booking->airline_pnr ? $booking->airline_pnr : $booking->gk_pnr }}. Por favor, consulte los detalles
        a continuación.
    </p>
    <p>
