<p>Estimado/a {{ $booking->customer_name ?? 'Pasajero' }},</p>
<p>¡Saludos cordiales!</p>
<p>Según nuestra conversación y como acordamos, hemos realizado su reserva con
    {{ $booking->airline_name ?? ($booking->segments->first()?->airline_name ?? 'la aerolínea') }} bajo
...
<p>
    ...
    <b>{{ $booking->airline_name ?? ($booking->segments->first()?->airline_name ?? 'la aerolínea') }} /
        {{ $booking->agency_merchant_name ?? '' }}</b>
    a procesar los cargos mencionados anteriormente bajo sus respectivos comercios para cargar a mi
    tarjeta ******{{ $booking->cards->first()?->card_last_four ?? '****' }} por la reserva del
    itinerario a continuación con {{ $booking->airline_name ?? ($booking->segments->first()?->airline_name ?? 'la aerolínea') }}.
</p>
<p>
    Esta autorización de pago es por el monto indicado anteriormente y es válida para un solo uso. Certifico que
    soy <b>{{ $booking->customer_name ?? '' }}</b>, un usuario autorizado de esta tarjeta y
    que no disputaré el pago con mi compañía de tarjeta de crédito/débito/banco.
</p>
<p>
    Por favor, confirme su aceptación de los términos y acuerdo a la declaración respondiendo a este correo con
    'Estoy de acuerdo' o 'Autorizo'.
</p>

<h4>Descripción de Cargos:</h4>
                ...
                (optional($card->merchant)->merchant_name ??
                    (optional($card->merchant)->merchantname ??
                        (optional($card->merchant)->name ??
                            ($cardOrder == 1
                                ? $booking->airline_merchant_name ?? 'Comercio de Aerolínea'
                                : $booking->agency_merchant_name ??
                                    ($booking->agencymerchantname ?? 'Comercio de Agencia'))))));
    @endphp
...
<table style="width: 100%; border: 1px solid #000; border-collapse: collapse; margin: 16px 0;">
    <thead style="border: 1px solid #000;">
        <tr style="background-color: #f3f4f6; border-bottom: 1px solid #e5e7eb;">
            <th style="padding: 12px 16px; text-align: left; font-weight: 600;">No.</th>
...
<table style="width: 100%; border-collapse: collapse; margin: 16px 0; background-color: #f9fafb;">
    <tbody>
        ...
        <tr style="border-bottom: 1px solid #e5e7eb;">
            <td style="padding: 12px 16px; font-weight: 600; background-color: #f3f4f6;">Fecha de Expiración:</td>
