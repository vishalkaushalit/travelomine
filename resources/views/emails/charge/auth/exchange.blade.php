<h3>Authorization for {{ $booking->segments->first()?->airline_name ?? 'the airline' }}  Exchange Confirmation.</h3>

<p>Dear {{ $booking->customer_name ?? 'Passeneger' }},</p>
<p>Greetings of the day !!</p>
<p>As per our conversation and as agreed,  we have made the changes on reservation with 
...
<p>
    Total penalty for all passengers: 
    {{ $booking->currency ?? 'USD' }}
    {{ number_format($booking->amount_charged, 2) }} (including all taxes and fees).
</p>
...
<p>
    ...
    ******{{ $booking->cards->first()?->card_last_four ?? '****' }} card for the flight change on the below-mentioned itinerary with
     {{ $booking->segments->first()?->airline_name ?? 'the airline' }}.
</p>
<p>
